...
bool read_indexed_positions( const char *filename, std::vector<Point>& positions, std::vector<unsigned>& indices,std::vector<Point>& wtexcoords,  std::vector<int>& material_indices)
{  
    ...
    for(;;)
    {
        ...
        if(line[0] == 'v')
        {
            ...
            else if(line[1] == 't')     // texcoord x y
            {
                if(sscanf(line, "vt %f %f", &x, &y) != 2)
                    break;
                texcoords.push_back( Point(x, y, 0) );
            }
        }
        ...
        int counter = 0;
            ...
            for(line= line +1; ; line= line + next)
            {
                ...
                next= 0;
                int wn= 0;
                if(sscanf(line, " %d/%d/%d %n", &wp.back(), &wt.back(), &wn, &next) == 3) 
                    continue;
        ...
        for(unsigned v= 2; v +1 < wp.size(); v++)
            {
                ...
                for(unsigned i= 0; i < 3; i++)
                {
                    ...
                    auto found= remap.insert( std::make_pair(vertex(material_id, p, t, p), unsigned(remap.size())) );
                    if(found.second)
                    {
                        counter++;
                        // pas trouve, copie les nouveaux attributs
                        if(t != -1) wtexcoords.push_back(texcoords[t]);
