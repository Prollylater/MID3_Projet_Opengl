...
void processPatternPlus(ivec2 curr_pix, int sample_radius) {
...
void processPatternCross(ivec2 curr_pix, int sample_radius) {
    int left = curr_pix.x - sample_radius;
    int right = curr_pix.x + sample_radius;
    int up = curr_pix.y + sample_radius;
    int down = curr_pix.y - sample_radius;
...
void main() {
  ...
  if(phase == 1 ||  phase == 3){
       ...
       processPatternCross(curr_pix,pattern_radius);
  }
  if(phase == 2 ||  phase == 4 ){
        curr_pix.x += pattern_radius;  
        processPatternPlus(curr_pix,pattern_radius);
        curr_pix.x -= pattern_radius;  
        curr_pix.y += pattern_radius;  
        processPatternPlus(curr_pix,pattern_radius);
  }
