...
GLuint create_default_program( const GLenum primitives, const size_t positions, const size_t texcoords, const size_t normals )
{
    assert(positions);
...
{
    return create_default_program(primitives, positions.size(), texcoords.size(), normals.size());
}
