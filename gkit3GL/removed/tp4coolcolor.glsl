...
{   
    ...
    ligth_dircamspace = normalize((normalize(vMatrix* vec4(1.0,21.0,1.0,1.0))).xyz - fragment_positioncamspace);
...
float shadowCalculation(vec4 fragment_lightspace)
{
    ...
    float depth = texture(shadowsampler, proj_coords.xy).r - 0.001;
...
void main( )
{
   ...
   // fragment_color = vec4(texture(shadowsampler, UV).rgb, 1.0);
